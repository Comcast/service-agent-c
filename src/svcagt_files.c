...
static int goal_state_file_close__ (FILE **fp)
{
	fclose (*fp);
	*fp = NULL;
  return 0;
...
#ifdef SVCAGT_KEEP_GOALS_OPEN
int svcagt_goal_state_file_close (FILE **fp __attribute__((unused)))
{
...
{
	return goal_state_file_close__ (fp);
}
#endif
...
void svcagt_files_close (void)
{
	...
	exclude_fp = NULL;
#ifdef SVCAGT_KEEP_GOALS_OPEN
	goal_state_file_close__ (&goal_states_fp);
#endif
}
